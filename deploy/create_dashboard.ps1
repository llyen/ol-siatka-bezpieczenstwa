...
.DESCRIPTION
    ...
    definicję istniejącego elementu OL_SIA_Dashboard.

.EXAMPLE
    .\deploy\create_dashboard.ps1 -WorkspaceName OL-ZK-Demo-Siatka
#>
[CmdletBinding()]
param(
    [string]$WorkspaceName = 'OL-ZK-Demo-Siatka',
    [string]$DashboardName = 'OL_SIA_Dashboard',
    ...
    [string]$KqlDatabaseId = 'a27ff4b5-c68b-4cf8-a4e2-4295a3e9b7ec',
...
function New-StableGuid([string]$value) {
    $bytes = [Text.Encoding]::UTF8.GetBytes("ol-sia-dashboard-$value")
...
function Get-PrimaryPage([string]$headerPage) {
    $known = @('Obraz sytuacji', 'Gotowosc dzialow', 'Siatka i moduly', 'Procedury SPO', 'Matryca ryzyka')
...
function Get-VisualType([string]$number) {
    # Scenariusz nie ma wspolrzednych geograficznych - podzial administracyjny jest
    # resortowy, nie terytorialny - wiec nie ma tu ani jednej mapy. Ciezar wizualny
    # przenosza wykresy udzialowe i slupkowe, a tabele zostaja tam, gdzie decydent
    # musi zobaczyc konkretny wiersz: kto, ktory modul, jaki powod blokady.
    switch ($number) {
        '01' { 'multistat'; break }
        { $_ -in @('03', '06', '15') } { 'pie'; break }
        { $_ -in @('02', '05') } { 'line'; break }
        { $_ -in @('07', '09', '11', '16', '18', '20', '23', '24') } { 'column'; break }
        default { 'table' }
    }
}
...
function Get-VisualOptions([string]$number, [string]$visualType) {
    ...
    if ($visualType -eq 'pie') {
        $pie = switch ($number) {
            '03' { @{ x = 'status'; y = 'Deklaracje' } }
            '06' { @{ x = 'status'; y = 'Zadania' } }
            '15' { @{ x = 'Rola';   y = 'Wpisy' } }
            default { @{ x = 'Kategoria'; y = 'Wartosc' } }
        }
        return @{
            xColumn = $pie.x
            yColumns = @($pie.y)
    ...
    $axis = switch ($number) {
        '02' { @{ xColumn = 'timestamp'; yColumns = @('Deklaracje'); seriesColumns = @('status'); xColumnTitle = 'Czas' } }
        '05' { @{ xColumn = 'timestamp'; yColumns = @('Zdarzenia'); seriesColumns = @('task_module_name'); xColumnTitle = 'Czas' } }
        '07' { @{ xColumn = 'Dzial'; yColumns = @('Brakujace', 'Wiodace'); seriesColumns = $null; xColumnTitle = 'Dzial administracji' } }
        '09' { @{ xColumn = 'Dzial'; yColumns = @('Wiodace', 'Zadania'); seriesColumns = $null; xColumnTitle = 'Dzial administracji' } }
        '11' { @{ xColumn = 'Resort'; yColumns = @('Zadania'); seriesColumns = @('status'); xColumnTitle = 'Resort' } }
        '16' { @{ xColumn = 'Modul'; yColumns = @('Dzialy', 'Wiodace'); seriesColumns = $null; xColumnTitle = 'Modul zadaniowy' } }
        '18' { @{ xColumn = 'Faza'; yColumns = @('Wpisy'); seriesColumns = @('Zagrozenie'); xColumnTitle = 'Faza ZK' } }
        '20' { @{ xColumn = 'Dzial'; yColumns = @('Min_SLA_h', 'Sredni_SLA_h'); seriesColumns = $null; xColumnTitle = 'Dzial administracji' } }
        '23' { @{ xColumn = 'Kategoria'; yColumns = @('Zagrozenia', 'Max_ryzyko'); seriesColumns = $null; xColumnTitle = 'Kategoria zagrozenia' } }
        '24' { @{ xColumn = 'Zagrozenie'; yColumns = @('Ryzyko'); seriesColumns = $null; xColumnTitle = 'Zagrozenie' } }
...
function New-DashboardJson($queries, [string]$schemaVersion, [switch]$Minimal) {
    $pageNames = @('Obraz sytuacji', 'Gotowosc dzialow', 'Siatka i moduly', 'Procedury SPO', 'Matryca ryzyka')
...
if ($dashboardQueries.Count -ne 24) { throw "Oczekiwano 24 zapytań, znaleziono $($dashboardQueries.Count)." }
    ...
    try {
        ...
        if ($existing) {
            ...
            $body = @{
                ...
                description = 'Real-Time Dashboard demo: siatka bezpieczenstwa i koordynacja miedzyresortowa'
